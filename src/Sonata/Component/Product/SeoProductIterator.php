...
class SeoProductIterator implements SourceIteratorInterface
{
     ...
     */
    public function __construct(EntityManagerInterface $entityManager, $class, RouterInterface $router, $routeName)
    {
        $tableName = $entityManager->getClassMetadata($class)->table['name'];

        $dql = "SELECT p.id as productId, p.slug as slug,  p.updated_at as lastmod, 'weekly' as changefreq, '0.5' as priority "
            ."FROM ".$tableName." p "
            ."WHERE p.enabled = 1";

        $source = new DoctrineDBALConnectionSourceIterator($entityManager->getConnection(), $dql);
