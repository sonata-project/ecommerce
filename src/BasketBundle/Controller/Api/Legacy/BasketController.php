...
class BasketController
{
     ...
     * @Rest\QueryParam(name="count", requirements="\d+", default="10", description="Number of baskets per page")
    ...
    protected function getBasket($id)
    {
        ...
        if (null === $basket) {
            throw new NotFoundHttpException(sprintf('Basket not found for identifier %s.', var_export($id, true)));
        }
    ...
    protected function getBasketElement($id)
    {
        ...
        if (null === $basketElement) {
            throw new NotFoundHttpException(sprintf('Basket element not found for identifier %s.', var_export($id, true)));
        }
    ...
    protected function checkExistingCustomerBasket($customerId): void
    {
        ...
        if ($basket instanceof BasketInterface) {
            throw new HttpException(400, sprintf('Customer (%s) already has a basket', $customerId));
        }
    }
}
